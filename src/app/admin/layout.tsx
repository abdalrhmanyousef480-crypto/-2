...
import { Store, Tag, FolderTree, FileText, LogOut, ExternalLink } from "lucide-react";
    ...
    <div className="min-h-screen flex" dir="rtl">
      <aside className="w-64 bg-primary text-white/85 flex flex-col shrink-0">
        <div className="p-5 flex items-center gap-2 border-b border-white/10">
          ...
          <span className="font-display font-bold text-white">كوبونيتا</span>
        </div>
        <nav className="flex-1 p-3">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-md text-sm font-medium hover:bg-white/10 hover:text-white transition-colors"
            >
